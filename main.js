...
try {
    ...
    const userWithQ = users.find(user => user.firstname.endsWith('q'));
    if (userWithQ) {
...
try {
    ...
    users.forEach(user => console.log(`User: ${user.firstname}, Age: ${user.age}`));
    const usersOver100 = users.filter(user => user.age > 100);
    if (usersOver100.length > 0) {
        usersOver100.forEach(user => console.log(`${user.firstname} is older than 100 and is ${user.age} years old.`));
    } else {
...
function findUserById(userId) {
    try {
        ...
        if (user) {
            console.log(`User found: ${user.firstname}, Age: ${user.age}`);
        } else {
...
function checkAddressById(userId) {
    try {
        const user = users.find(user => user.id === userId);
        if (user) {
            if (user.address) {
                console.log(`User with id ${userId} has address: ${user.address}`);
            } else {
                throw new Error(`User with id ${userId} does not have an address.`);
            }
        } else {
            throw new Error(`User with id ${userId} not found.`);
        }
...
// weight ve ya height her hansisa birisi yoxdursa boy ve ya kilo melumati eksikdi deye error versin
function checkWeightHeightById(userId) {
    try {
        const user = users.find(user => user.id === userId);
        if (user) {
            if (user.weight === null || user.height === null) {
                throw new Error(`Boy ve ya kilo melumati eksikdir. İstifadeci ID: ${userId}`);
            } else {
                console.log(`User with id ${userId} has weight: ${user.weight} kg and height: ${user.height} cm.`);
            }
        } else {
            throw new Error(`User with id ${userId} not found.`);
        }
...
checkWeightHeightById(2); 
checkWeightHeightById(3);
checkWeightHeightById(4);
checkWeightHeightById(5); 
